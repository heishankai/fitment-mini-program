<template>
  <view class="construction-node">
    <node-description title="增加的施工节点">
      <timeline-item
        v-for="(item, i) in construction_nodes"
        :key="subNodeKey(item, i)"
        :completed="true"
        :title="`${item?.work_kind_name}施工`"
        :is-last="i === construction_nodes.length - 1"
      >
...
const props = defineProps<{
  order_details: any
  construction_nodes: any[]
}>()

const subNodeKey = (item: any, i: number): string | number =>
  [item?.work_group_id, item?.assigned_craftsman_id, item?.id].filter((v) => v != null).join('-') ||
  i
